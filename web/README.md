...
一般端末とキオスク端末では、完全に別のホーム画面を開きます。振り分けは共通レイアウト `app/(protected)/(app)/layout.tsx` が行い、リクエストの `x-api-key` が有効で、認証ユーザーがキオスクとして登録されていれば（`isKiosk()`、`lib/better-auth/session.ts`） `/kiosk` へ転送します。逆に一般端末が `/kiosk` 以下を開いた場合は `kiosk/layout.tsx` で `/` へ戻します。

- `/`（`app/(protected)/(app)/page.tsx`）: ブラウザーからログインしたときのホームです。キオスクでなくても使う普段づかいの画面で、同じサービスをスマートフォンやパソコンからも利用できる想定です。
- `/generic` 以下: 一般メニューの各サービス（伝言板・買い物リスト・予定と天気・家の状態・家電の操作・タイマー）。`/admin` と同じくフォルダごとに 1 ページを置きます。サービスの定義（パス・名前・説明・アイコン）は `lib/generic/services.ts` にまとめ、サイドバーとホームの一覧が同じ定義を参照します。`/generic/kiosks` は一般端末向けの見守り・声かけ画面です。その他のサービスは現在サンプル表示です。`/generic` 自体は `/` へ転送します。
...
- 登録済みキオスクは `x-api-key` 認証で `/kiosk` を開き、「カメラ・スピーカーを開始」を押してカメラ・マイクを許可します。配信中はログイン済みの一般端末・他のキオスクから接続できます。ページを閉じるか「配信を停止」で配信を終了します。
...
- 音声の自動再生をブラウザーが制限した場合は「スピーカーの音声を有効にする」を押します。端末キーが有効でも配信中とは限りません。接続画面で参加者と映像を確認してください。
- トークンは接続先ルームと送信可能なメディアに限定され、有効期間は10分です。端末キーの停止は新規接続を拒否しますが、すでに接続済みの LiveKit セッションを強制切断するものではありません。

実機確認: 2台の登録済みキオスクで配信を開始し、一般ブラウザーからの閲覧・声かけ、キオスク間のビデオ通話、終了後のカメラ解放、カメラ権限拒否と再接続を確認してください。Android WebView のカメラ・マイク権限と API キーヘッダー付与は端末アプリ側でも必要です。


## 画面と共通処理の配置

- `app/(protected)/layout.tsx`: 認証の共通境界。
- `app/(protected)/(app)/`: 一般端末のホーム `/` と一般サービス `generic/`、管理画面 `admin/`。テーマ変更・ログアウトを含む共通サイドバーを使用します。
- `app/(protected)/kiosk/`: キオスク専用の起点 `/kiosk`。専用レイアウトで端末種別を確認します。
- `components/app/`: ナビゲーションとサイドバー。モバイルでは標準の `<dialog>` で画面上部にメニューを重ね、背景操作・スクロールを止めます。背景クリック、Escape、リンク選択で閉じます。
- `components/kiosk/`: 両方の画面から使う映像・音声UI。
- `lib/kiosk/`: 見守り画面向けの端末データと共有型。キー管理は既存の `lib/admin/`、通話権限は `lib/livekit/` が担当します。
- `lib/routes.ts`: 端末種別の起点と共通の見守りパス。メニュー名・説明・提供状態は `lib/generic/services.ts` に集約します。
