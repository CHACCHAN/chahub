...
```tsx
import { Card, List, ListItem, Input, Button, Badge, TextLink } from "@/components/ui";
...
色はインディゴ(アクセント)と zinc(中立)だけを使い、`dark:` も必ず併記します。
機能固有の組み合わせ(例: 管理画面のユーザー表示)は `components/<feature>/` に置きます。
